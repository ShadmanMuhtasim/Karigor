        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {categories.map((cat) => (
              <Card
                key={cat.id}
                className="card-lift bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-indigo-500/50 dark:hover:border-sky-500/50 rounded-2xl shadow-sm"
              >
                <CardHeader>
                  <CardTitle className="flex items-center gap-3 text-base">
                    ...
                    <span className="w-8 h-8 rounded-xl bg-indigo-50 dark:bg-indigo-950/80 text-indigo-600 dark:text-sky-400 flex items-center justify-center shrink-0" style={{ display: cat.iconUrl ? 'none' : 'inline-flex' }}>
