  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-xl font-black text-gray-900 dark:text-white">{t('admin.bookings.title')}</h3>
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl">
          <ClipboardListIcon className="w-10 h-10 mx-auto mb-3 text-gray-400" />
          <h4 className="text-base font-bold text-gray-900 dark:text-white">{t('admin.bookings.noBookingsFound')}</h4>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('admin.bookings.noBookingsFound')}</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-800 uppercase tracking-wider text-[10px] font-bold text-gray-500 dark:text-gray-400">
                <tr>
                  <th className="py-3.5 px-4 sm:px-6">{t('admin.bookings.bookingServiceColumn')}</th>
                  <th className="py-3.5 px-4">{t('admin.bookings.customerColumn')}</th>
                  <th className="py-3.5 px-4">{t('admin.bookings.artisanColumn')}</th>
                  <th className="py-3.5 px-4">{t('admin.bookings.priceColumn')}</th>
                  <th className="py-3.5 px-4">{t('admin.bookings.dateColumn')}</th>
                  <th className="py-3.5 px-4">{t('admin.bookings.statusColumn')}</th>
                  <th className="py-3.5 px-4 sm:px-6 text-right">{t('common.details')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={b.id} className="table-row-hover">
                    <td className="py-4 px-4 sm:px-6">
                      <div>
                        <span className="font-bold text-gray-900 dark:text-white text-xs flex items-center gap-1.5">
                          <WrenchIcon className="w-3.5 h-3.5 text-gray-500 shrink-0" />
                          <span>{b.categoryName}</span>
                        </span>
                        <p className="text-[11px] text-gray-500 dark:text-gray-400 font-mono">#{b.id} (Req #{b.serviceRequestId})</p>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        {b.hasReview && (
                          <span className="px-2 py-0.5 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 rounded-full text-[10px] font-bold flex items-center gap-1">
                            <StarIcon className="w-3 h-3 fill-amber-400 text-amber-400" />
                            <span>{b.reviewRating}</span>
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4 sm:px-6 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 font-bold rounded-xl text-xs btn-press cursor-pointer inline-flex items-center gap-1"
                      >
      ...
      {selectedBooking && (
        ...
        >
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-5 animate-modal-pop">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-3">
              <div>
                <h4 className="text-base font-black text-gray-900 dark:text-white">
                  {t('admin.bookings.modalTitle')} #{selectedBooking.id}
                </h4>
                <p className="text-xs text-gray-500">#{selectedBooking.serviceRequestId}</p>
              </div>
              <button
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-gray-50 dark:bg-gray-800/60 rounded-2xl flex items-center justify-between">
                <div>
                  <span className="text-[10px] uppercase font-bold text-gray-400 block">{t('admin.overview.categories')}</span>
                  <span className="font-bold text-gray-900 dark:text-white text-sm flex items-center gap-1 mt-0.5">
                    <WrenchIcon className="w-3.5 h-3.5 text-gray-500" />
                    <span>{selectedBooking.categoryName}</span>
                  </span>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-bold text-gray-400 block text-right">{t('admin.bookings.priceColumn')}</span>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-gray-50 dark:bg-gray-800/60 rounded-2xl">
                  <span className="text-[10px] uppercase font-bold text-gray-400 block">{t('admin.bookings.customerColumn')}</span>
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-800/60 rounded-2xl">
                  <span className="text-[10px] uppercase font-bold text-gray-400 block">{t('admin.bookings.artisanColumn')}</span>
              ...
              {selectedBooking.address && (
                <div className="p-3 bg-gray-50 dark:bg-gray-800/60 rounded-2xl">
                  <span className="text-[10px] uppercase font-bold text-gray-400 block">{t('admin.bookings.customerAddress')}</span>
                  <p className="font-medium text-gray-800 dark:text-gray-200 mt-1 flex items-center gap-1">
                    <MapPinIcon className="w-3.5 h-3.5 text-gray-500 shrink-0" />
              ...
              <div className="p-3 bg-gray-50 dark:bg-gray-800/60 rounded-2xl flex items-center justify-between">
                <div>
                  <span className="text-[10px] uppercase font-bold text-gray-400 block">{t('admin.bookings.dateColumn')}</span>
                  <p className="font-semibold text-gray-800 dark:text-gray-200 mt-0.5 flex items-center gap-1">
                    <CalendarIcon className="w-3.5 h-3.5 text-gray-500 shrink-0" />
                    <span>{new Date(selectedBooking.scheduledDate).toLocaleString()}</span>
                  </p>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-bold text-gray-400 block text-right">{t('admin.bookings.statusColumn')}</span>
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setSelectedBooking(null)}
                className="px-4 py-2 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 text-gray-800 dark:text-gray-200 font-bold rounded-xl text-xs btn-press cursor-pointer"
              >
                {t('common.close')}
              </button>
            </div>
